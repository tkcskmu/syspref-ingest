...
mod tests {
    ...
    #[tokio::test]
    async fn envelope_job_not_found() {
        let id = Uuid::new_v4();
    ...
    #[tokio::test]
    async fn envelope_missing_file() {
        let (status, body) = response_envelope(AppError::MissingFile).await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert_eq!(body["error"]["code"], "missing_file");
        assert_eq!(body["error"]["message"], "file field is missing");
    }

    #[tokio::test]
    async fn envelope_missing_profile() {
        let (status, body) = response_envelope(AppError::MissingProfile).await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert_eq!(body["error"]["code"], "missing_profile");
        assert_eq!(body["error"]["message"], "profile field is missing");
    }

    #[tokio::test]
    async fn envelope_artifact_not_ready() {
        let (status, body) = response_envelope(AppError::ArtifactNotReady).await;
        assert_eq!(status, StatusCode::CONFLICT);
        assert_eq!(body["error"]["code"], "artifact_not_ready");
        assert_eq!(body["error"]["message"], "artifact is not ready");
    }

    #[tokio::test]
    async fn envelope_job_failed() {
        let (status, body) = response_envelope(AppError::JobFailed).await;
        assert_eq!(status, StatusCode::CONFLICT);
        assert_eq!(body["error"]["code"], "job_failed");
        assert_eq!(body["error"]["message"], "job failed");
    }
